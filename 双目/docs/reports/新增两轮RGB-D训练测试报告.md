...
旧模型 `models/archive/rgbd/geometry-rgbd-multipose-v3.npz` 的 223 个样本被保留，新模型追加 66 个样本：

```powershell
.\.venv\Scripts\python.exe -m sorting_vision.cli geometry-rgbd-train `
  --data-root data/rgbd-reviewed-multi-02-03 `
  --base-model models/archive/rgbd/geometry-rgbd-multipose-v3.npz `
  --output models/experimental/rgbd/geometry-rgbd-multipose-v4-next.npz
```
