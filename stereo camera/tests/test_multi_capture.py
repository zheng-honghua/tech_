...
def test_multi_capture_stream_profile_defers_to_d435if_config():
    args = cli.build_parser().parse_args([
        "rgbd-multi-capture", "--batch-id", "multi-01",
        "--composition", "三棱柱:1",
    ])
    assert (args.color_width, args.color_height) == (None, None)
    assert (args.depth_width, args.depth_height, args.fps) == (None, None, None)
    camera = cli.load_config("config/d435if-initial.yaml").camera
    assert (camera.realsense_color_width, camera.realsense_color_height) == (1920, 1080)
    assert (camera.realsense_depth_width, camera.realsense_depth_height) == (848, 480)
    assert camera.realsense_fps == 30
