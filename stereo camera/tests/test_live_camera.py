...
def test_uvc_source_applies_requested_fixed_imaging_controls():
    image = np.full((10, 12, 3), 70, np.uint8)
...
def test_d435if_source_uses_new_metadata_and_frame_prefix():
    rs = FakeRS()
    source = RealSenseD435IFSource(
        depth_width=4,
        depth_height=3,
        color_width=4,
        color_height=3,
    ...
    frame = source.read()
    assert frame.frame_id == "d435if-000000007"
    assert source.capture_metadata()["camera_model"] == "Intel RealSense D435if"
    source.close()


def test_realsense_factory_uses_d435if_config(monkeypatch):
    captured = {}

    class StubRealSenseSource:
        def __init__(self, **kwargs):
            captured.update(kwargs)

    monkeypatch.setattr(cli, "RealSenseSource", StubRealSenseSource)
    args = Namespace(
        source="realsense",
        width=None,
        height=None,
    ...
    cli._make_camera_source(args, load_config("config/d435if-initial.yaml"))
    assert captured == {
        "width": 1920,
        "height": 1080,
        "fps": 30,
        ...
        "color_width": 1920,
        "color_height": 1080,
        "camera_model": "Intel RealSense D435if",
        "frame_prefix": "d435if",
    }
