...
from sorting_vision.interlock import DualMotionInterlock, MotionInterlock, RunState
...
def test_dual_interlock_waits_for_both_available_views_to_stabilize():
    clock = Clock()
    config = MotionInterlockConfig(
        min_settle_ms=0,
        discard_frames=0,
        stable_frames=1,
        frame_diff_threshold=2.5,
    ...
    interlock = DualMotionInterlock(config, clock)
    primary = np.full((48, 64, 3), 100, np.uint8)
    side_a = np.full((48, 64, 3), 20, np.uint8)
    side_b = np.full((48, 64, 3), 200, np.uint8)
    interlock.motion_start()
    interlock.motion_stop()
    interlock.observe_pair(primary, side_a)
    assert interlock.observe_pair(primary, side_b) == RunState.SETTLING
    assert interlock.observe_pair(primary, side_b) == RunState.READY
