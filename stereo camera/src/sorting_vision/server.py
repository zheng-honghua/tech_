...
from .camera import ColorSource, RGBDSource, SynchronizedFramePair
...
class VisionService3D:
    def __init__(
        ...
        interlock: MotionInterlock | DualMotionInterlock | None = None,
        mode: str | None = None,
    ) -> None:
    ...
    def update(self) -> list[Any]:
        ...
        with self._lock:
            ...
            self._latest_frame = (
                frame.primary if isinstance(frame, SynchronizedFramePair) else frame
            )
            if isinstance(frame, SynchronizedFramePair):
                if isinstance(self.interlock, DualMotionInterlock):
                    state = self.interlock.observe_pair(
                        frame.primary.color_bgr,
                        None if frame.side is None else frame.side.color_bgr,
                    )
                else:
                    state = self.interlock.observe(frame.primary.color_bgr)
            else:
            ...
            if state != RunState.READY:
                ...
                return []
            if isinstance(frame, SynchronizedFramePair):
                process_pair = getattr(self.pipeline, "process_pair", None)
                if process_pair is None:
                    raise TypeError("dual source requires a dual-capable pipeline")
                self._latest_results = process_pair(frame)
            else:
        ...
        health.update(
            {
                ...
                "can_pick": ready
                and self.mode in {"RGBD", "RGBD_DUAL"}
