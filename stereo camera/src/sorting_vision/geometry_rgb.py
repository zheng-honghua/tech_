...
class GeometryRGBModel:
    ...
    def predict_feature(self, raw_feature: np.ndarray) -> tuple[str, float, dict[str, float | str]]:
        ...
        distance_values = np.asarray([class_distances[label] for label in sorted(class_distances)], np.float64)
        distance_scale = max(float(np.median(np.abs(distance_values - np.median(distance_values)))), 0.05)
        logits = -(distance_values - float(distance_values.min())) / distance_scale
        probabilities = np.exp(logits - float(logits.max()))
        probabilities /= max(float(probabilities.sum()), 1e-12)
        self.last_class_scores = {
            label: float(probabilities[index])
            for index, label in enumerate(sorted(class_distances))
        }
        ...
        self.last_rejection_reason = str(diagnostics["reason"])
    ...
    def predict(self, image_bgr: np.ndarray, mask: np.ndarray | None = None) -> tuple[str, float, dict[str, float | str]]:
        ...
        if prepared is None:
            self.last_class_scores = {}
            self.last_rejection_reason = "object_not_found"
    ...
    def predict_preprocessed(
        ...
        if prepared.candidate_count != 1:
            self.last_class_scores = {}
            self.last_rejection_reason = "multiple_objects"
            return "unknown", 0.0, {
        ...
        if self.feature_set == "edge-topology":
            ...
            if topology.reason != "accepted":
                self.last_class_scores = {}
                self.last_rejection_reason = topology.reason
                return "unknown", 0.0, {
            ...
            if len(topology.merged_lines) >= 14 and not topology.junctions:
                self.last_class_scores = {}
                self.last_rejection_reason = "topology_conflict"
                return "unknown", 0.0, {
