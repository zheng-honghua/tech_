    ...
    def predict_features(self, features: np.ndarray) -> tuple[str, float, str]:
        ...
        if supplied_names is not None and self.feature_names != supplied_names:
            if not all(name in supplied_names for name in self.feature_names):
                ...
                self.last_rejection_reason = "invalid_features"
        ...
        if not np.any(finite):
            ...
            self.last_rejection_reason = "invalid_features"
            return "unknown", 0.0, "invalid_features"
        usable = distances.copy().astype(np.float64)
        usable[~finite] = float(np.max(usable[finite]) + 4.0)
        distance_scale = max(float(np.median(np.abs(usable - np.median(usable)))), 0.05)
        logits = -(usable - float(usable.min())) / distance_scale
        probability_values = np.exp(logits - float(logits.max()))
        probability_values /= max(float(probability_values.sum()), 1e-12)
        self.last_class_scores = {
        ...
        if distance > float(self.thresholds[best]):
            self.last_rejection_reason = "distance_rejected"
        ...
        if margin_value < self.min_margin:
            self.last_rejection_reason = "margin_rejected"
