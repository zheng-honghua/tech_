...
class DualMotionInterlock:
    """Require both available camera images to settle before dual inference."""
    ...
    def __init__(
        ...
        self.primary = MotionInterlock(config, monotonic_ns)
        self.side = MotionInterlock(config, monotonic_ns)
        self._side_available = True

    @property
    def state(self) -> RunState:
        states = [self.primary.state]
        if self._side_available:
            states.append(self.side.state)
        if RunState.MOVING in states:
            return RunState.MOVING
        if RunState.SETTLING in states:
            return RunState.SETTLING
        return RunState.READY

    def motion_start(self) -> None:
        self.primary.motion_start()
        self.side.motion_start()

    def motion_stop(self) -> None:
        self.primary.motion_stop()
        self.side.motion_stop()

    def observe_pair(
        self, primary_bgr: np.ndarray, side_bgr: np.ndarray | None
    ) -> RunState:
        self.primary.observe(primary_bgr)
        self._side_available = side_bgr is not None
        if side_bgr is not None:
            self.side.observe(side_bgr)
        return self.state

    @property
    def status(self) -> str:
        statuses = [self.primary.status]
        if self._side_available:
            statuses.append(self.side.status)
        if "BUSY_MOVING" in statuses:
            return "BUSY_MOVING"
        if "MOTION_UNSTABLE" in statuses:
            return "MOTION_UNSTABLE"
        if "SETTLING" in statuses:
            return "SETTLING"
        return "OK"
