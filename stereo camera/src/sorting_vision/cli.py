...
from .interlock import DualMotionInterlock, MotionInterlock
...
def _make_interlock(config, dual: bool = False):
    interlock_type = DualMotionInterlock if dual else MotionInterlock
    return interlock_type(config.motion_interlock)
...
def _make_camera_source(args: argparse.Namespace, config):
    ...
    if args.source == "realsense":
        return RealSenseSource(
            ...
            depth_width=(
                getattr(args, "depth_width", None)
                or camera.realsense_depth_width
            ),
            depth_height=(
                getattr(args, "depth_height", None)
                or camera.realsense_depth_height
            ),
            color_width=(
                getattr(args, "color_width", None)
                or camera.realsense_color_width
            ),
            color_height=(
                getattr(args, "color_height", None)
                or camera.realsense_color_height
            ),
        ...
        try:
            ...
            side_kwargs = dict(
                camera_index=(
                    ...
                    else config.dual_view.side_camera_index
                ),
                ...
                exposure=config.dual_view.side_exposure,
                auto_white_balance=config.dual_view.side_auto_white_balance,
                white_balance=config.dual_view.side_white_balance,
...
def _make_live_service(args: argparse.Namespace, config):
    ...
    try:
        ...
        if calibration is None and background_frame is None:
            raise ValueError(
                "RealSense mode requires --background-dir or --rgbd-calibration; "
        ...
        if args.source == "dual":
            dual_cfg = config.dual_view
            if getattr(args, "platform_id", None):
                dual_cfg = replace(dual_cfg, platform_id=args.platform_id)
            ...
            side_background = (
                _read_image(side_background_path)
...
def _run_rgbd_capture(args: argparse.Namespace) -> int:
    ...
    config = load_config(args.config)
    source = _make_camera_source(args, config)
    camera_label = config.camera.realsense_model
    saved = 0
    frame = None
    print(f"{camera_label} capture: SPACE=save one RGB-D bundle, q=quit")
    try:
        ...
        while args.count <= 0 or saved < args.count:
            ...
            if not args.headless:
                ...
                cv2.imshow(f"{camera_label} RGB-D dataset capture", canvas)
                key = cv2.waitKey(1) & 0xFF
                if key == ord("q"):
                    break
                should_save = key == 32
...
def _run_rgbd_capture_assistant(args: argparse.Namespace) -> int:
    ...
    print(f"{camera_label} dataset assistant")
    ...
    try:
        ...
        while True:
            ...
            cv2.imshow(
                f"{camera_label} RGB-D capture assistant",
...
def _run_rgbd_multi_capture(args: argparse.Namespace) -> int:
    ...
    print(f"{camera_label} multi-object batch capture")
    ...
    try:
        ...
        while True:
            ...
            if not args.headless:
                cv2.imshow(
                    f"{camera_label} multi-object batch capture",
...
def _load_calibration_image_pairs(
    primary_dir: str | Path, side_dir: str | Path
) -> tuple[list[np.ndarray], list[np.ndarray]]:
    suffixes = {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff"}
    primary_paths = sorted(
        path for path in Path(primary_dir).iterdir() if path.suffix.lower() in suffixes
    )
    side_paths = sorted(
        path for path in Path(side_dir).iterdir() if path.suffix.lower() in suffixes
    )
    primary_by_stem = {path.stem: path for path in primary_paths}
    side_by_stem = {path.stem: path for path in side_paths}
    common = sorted(set(primary_by_stem) & set(side_by_stem))
    if common:
        pairs = [(primary_by_stem[stem], side_by_stem[stem]) for stem in common]
    elif len(primary_paths) == len(side_paths):
        pairs = list(zip(primary_paths, side_paths))
    else:
        raise ValueError(
            "calibration directories need matching stems or equal sorted image counts"
        )
    primary_images = [_read_image(first) for first, _ in pairs]
    side_images = [_read_image(second) for _, second in pairs]
    return primary_images, side_images


def _run_dual_calibrate(args: argparse.Namespace) -> int:
    primary_images, side_images = _load_calibration_image_pairs(
        args.primary_dir, args.side_dir
    )
    calibration = calibrate_charuco_pairs(
        primary_images,
        side_images,
        ...
        tray_pose_index=args.tray_pose_index,
    )
    background = load_rgbd_frame(args.background_dir)
    rgbd_workspace = VisionPipeline3D(
        config=load_config(args.config), background_frame=background
    ).calibration
    if (background.intrinsics.width, background.intrinsics.height) != calibration.primary_image_size:
        raise ValueError("RGB-D background dimensions do not match ChArUco primary images")
    calibration = replace(
        calibration,
        tray_plane_primary=rgbd_workspace.tray_plane_camera,
    )
    calibration.save(args.output)
...
def _run_dual_capture(args: argparse.Namespace) -> int:
    ...
    root = Path(args.dataset_root) / args.platform_id / args.batch_id
    count = 0
    side_background_frames: list[np.ndarray] = []
    try:
        while count < args.count:
            ...
            if not args.headless:
                side = (
                    np.zeros_like(pair.primary.color_bgr)
                ...
                cv2.imshow("dual-primary-side", np.hstack((pair.primary.color_bgr, side)))
                key = cv2.waitKey(1) & 0xFF
                if key == ord("q"):
                    break
                capture = key == ord("c")
            ...
            if not pair.synchronized:
                print(
                    f"skipped unsynchronised pair: delta_ms={pair.pair_delta_ms}, error={pair.side_error}",
                    file=sys.stderr,
                )
                continue
            count += 1
            sample = root / f"sample-{count:04d}"
            save_synchronized_pair(
                pair,
                sample,
                args.platform_id,
        ...
        if not args.headless:
            cv2.destroyAllWindows()
    if args.side_background_output and side_background_frames:
        target = Path(args.side_background_output)
        target.parent.mkdir(parents=True, exist_ok=True)
        median_background = np.median(
            np.stack(side_background_frames), axis=0
        ).astype(np.uint8)
        if not cv2.imwrite(str(target), median_background):
            raise OSError(f"failed to save side background: {target}")
...
def _run_dual_review(args: argparse.Namespace) -> int:
    ...
    if background_frame is None and rgbd_calibration is None:
        raise ValueError("dual-review requires --background-dir or --rgbd-calibration")
    pipeline = VisionPipeline3D(
        config=config,
        calibration=rgbd_calibration,
        background_frame=background_frame,
        shape_model=_make_depth_shape_model(args),
    )
    ...
    for metadata_path in sorted(samples_root.rglob("metadata.json")):
        ...
        if metadata.get("calibration_hash") != dual_calibration.calibration_hash:
            raise ValueError(f"sample calibration hash mismatch: {sample_dir}")
        if metadata.get("platform_id") != args.platform_id:
            raise ValueError(f"sample platform mismatch: {sample_dir}")
        pair = load_synchronized_pair(sample_dir)
        results = pipeline.process_pair(pair)
        relative = sample_dir.relative_to(samples_root)
        target = annotations_root / relative
        target.mkdir(parents=True, exist_ok=True)
        cv2.imwrite(
            str(target / "primary-annotated.png"),
            pipeline.annotate(pair.primary, results),
        )
        cv2.imwrite(
            str(target / "side-annotated.png"),
            fusion.annotate_side(pair.side.color_bgr, results),
        )
        (target / "dual-results-v2.json").write_text(
...
def _run_dual_evaluate(args: argparse.Namespace) -> int:
    records = [
        json.loads(line)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    dual_calibrate = subparsers.add_parser(
        "dual-calibrate",
        help="calibrate ChArUco intrinsics and T_side_from_primary",
    )
    dual_calibrate.add_argument("--primary-dir", required=True)
    dual_calibrate.add_argument("--side-dir", required=True)
    dual_calibrate.add_argument(
        "--background-dir",
        required=True,
        help="empty-tray RGB-D frame used to fit and bind the tray depth plane",
    )
    dual_calibrate.add_argument(
        "--platform-id", choices=("temporary", "competition"), required=True
    )
    dual_calibrate.add_argument("--squares-x", type=int, default=7)
    dual_calibrate.add_argument("--squares-y", type=int, default=5)
    dual_calibrate.add_argument("--square-length-mm", type=float, default=20.0)
    dual_calibrate.add_argument("--marker-length-mm", type=float, default=14.0)
    dual_calibrate.add_argument("--dictionary", default="DICT_5X5_100")
    dual_calibrate.add_argument(
        "--tray-pose-index",
        type=int,
        required=True,
        help="zero-based paired image index where the board lies flat in the tray",
    )
    dual_calibrate.add_argument("--output", required=True)
    dual_calibrate.set_defaults(func=_run_dual_calibrate)

    dual_capture = subparsers.add_parser(
        "dual-capture", help="capture synchronized primary RGB-D and side RGB samples"
    )
    dual_capture.add_argument("--dataset-root", required=True)
    dual_capture.add_argument("--batch-id", required=True)
    ...
    dual_capture.add_argument("--width", type=int)
    dual_capture.add_argument("--height", type=int)
    dual_capture.add_argument("--fps", type=int)
    dual_capture.add_argument("--depth-width", type=int)
    dual_capture.add_argument("--depth-height", type=int)
    dual_capture.add_argument("--color-width", type=int)
    dual_capture.add_argument("--color-height", type=int)
    dual_capture.add_argument("--side-width", type=int)
    dual_capture.add_argument("--side-height", type=int)
    dual_capture.add_argument("--side-fps", type=int)
    dual_capture.add_argument("--dual-calibration")
    dual_capture.add_argument(
        "--side-background-output",
        help="write the median side image of captured empty-tray pairs",
    )
    ...
    dual_review.add_argument("--samples-root", required=True)
    dual_review.add_argument("--output-dir", required=True)
    ...
    dual_review.add_argument("--columns", type=int, default=1)
    ...
    dual_evaluate.add_argument("--records", required=True)
    dual_evaluate.add_argument("--output")
    dual_evaluate.set_defaults(func=_run_dual_evaluate)

    dual_temperature = subparsers.add_parser(
        "dual-temperature-fit",
        help="fit top and side probability temperatures on a calibration split",
    )
    ...
    rgbd_capture.add_argument("--color-width", type=int)
    rgbd_capture.add_argument("--color-height", type=int)
    rgbd_capture.add_argument("--depth-width", type=int)
    rgbd_capture.add_argument("--depth-height", type=int)
    rgbd_capture.add_argument("--fps", type=int)
    ...
    rgbd_assistant = subparsers.add_parser(
        ...
        help="interactively capture and label a complete RealSense geometry batch",
    )
    ...
    rgbd_assistant.add_argument("--color-width", type=int)
    rgbd_assistant.add_argument("--color-height", type=int)
    rgbd_assistant.add_argument("--depth-width", type=int)
    rgbd_assistant.add_argument("--depth-height", type=int)
    rgbd_assistant.add_argument("--fps", type=int)
    ...
    rgbd_multi = subparsers.add_parser(
        "rgbd-multi-capture",
        help="batch-capture labelled multi-object RealSense test scenes",
    )
    ...
    rgbd_multi.add_argument("--color-width", type=int)
    rgbd_multi.add_argument("--color-height", type=int)
    rgbd_multi.add_argument("--depth-width", type=int)
    rgbd_multi.add_argument("--depth-height", type=int)
    rgbd_multi.add_argument("--fps", type=int)
    ...
    serve.add_argument("--source", choices=("file", "uvc", "realsense", "dual"), default="file")
    ...
    serve.add_argument("--side-camera-index", type=int)
    serve.add_argument("--side-width", type=int)
    serve.add_argument("--side-height", type=int)
    serve.add_argument("--side-fps", type=int)
    ...
    camera_live = subparsers.add_parser(
        "camera-live", help="preview a UVC, RealSense, or dual camera source with motion interlock"
    )
    camera_live.add_argument("--source", choices=("uvc", "realsense", "dual"), required=True)
    ...
    camera_live.add_argument("--side-camera-index", type=int)
    camera_live.add_argument("--side-width", type=int)
    camera_live.add_argument("--side-height", type=int)
    camera_live.add_argument("--side-fps", type=int)
