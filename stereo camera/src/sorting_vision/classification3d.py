...
class PrimitiveShapeClassifier3D:
    ...
    def _prediction(
        ...
        return LabelPrediction(
            ...
            features,
            (
                {label_id: float(np.clip(confidence, 0.0, 1.0)),
                 "unknown": float(np.clip(1.0 - confidence, 0.0, 1.0))}
                if label_id != "unknown"
                else {"unknown": 1.0}
            ),
            "accepted" if label_id != "unknown" else "distance_rejected",
        )
...
class HybridShapeClassifier3D:
    ...
    def classify(
        ...
        if baseline.features.get("point_count", 0) < 40:
            ...
            self.last_rejection_reason = "insufficient_points"
            return baseline
        if self.model is None:
            self.last_class_scores = {
                baseline.label_id: baseline.confidence,
                "unknown": max(0.0, 1.0 - baseline.confidence),
            }
            self.last_rejection_reason = (
                "accepted" if baseline.label_id != "unknown" else "distance_rejected"
            )
            return baseline
        ...
        self.last_class_scores = dict(getattr(self.model, "last_class_scores", {}))
        self.last_rejection_reason = str(
            getattr(self.model, "last_rejection_reason", "accepted" if label != "unknown" else "unknown")
        )
        valid_depth = (
            (crop_mask > 0)
        ...
        if metric_hexagonal_override:
            ...
            self.last_class_scores = {"hexagonal_prism": model_confidence}
        ...
        if agreement:
            ...
            return LabelPrediction(
                "unknown",
                self.cfg.shapes.get("unknown", "unknown"),
                float(np.clip(fused, 0.0, 1.0)),
                features,
                dict(self.last_class_scores),
                self.last_rejection_reason,
            )
        return LabelPrediction(
            label,
