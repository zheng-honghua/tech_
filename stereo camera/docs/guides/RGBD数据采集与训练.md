...
当前硬件已经从 D415 更换为 D435if。开始采集前先阅读 [D435if 迁移与重新标定](D435if迁移与重新标定.md)。原 D415 配置、空托盘、内外参和 v4 评估结果仅用于历史复现，不能直接当作 D435if 已验证结果。

## 1. 安装与连接

以下命令均在仓库根目录执行。先创建并激活虚拟环境，再关闭 RealSense Viewer、连接 D435if 并安装可选依赖：
...
使用 `config/d435if-initial.yaml` 时，采集程序默认使用彩色 `1920×1080@30`、深度 `848×480@30`，并把深度对齐到彩色图。这组 profile 已在当前 D435IF 上验证可启动；若更换设备或 profile，同一训练/验证/测试实验必须保持流配置一致。

## 2. 采集文件包

需要连续完成空托盘和全部类别时，推荐使用 [D435if 数据集拍摄助手](D435if数据集拍摄助手.md)中的 `rgbd-capture-assistant`；它支持按数字切换类别、计数恢复、画面稳定和深度质量检查。下面的 `rgbd-capture` 适合只采集一个指定类别。
...
.\.venv\Scripts\python.exe -m sorting_vision.cli `
  ...
  --dataset-root "data\rgbd-d435if" --batch-id d435if-pilot-01 --label 空托盘
```
...
.\.venv\Scripts\python.exe -m sorting_vision.cli `
  --config config\d435if-initial.yaml rgbd-capture `
  --dataset-root "data\rgbd-d435if" --batch-id d435if-pilot-01 --label 三棱柱
```

每按一次空格会创建一个独立文件夹：

```text
data/rgbd-d435if/d435if-pilot-01/triangular_prism/<sample-id>/
  color.png              # 彩色图
...
当前多姿态模型使用点云主轴尺寸、表面平面性、厚度/深度分布和外轮廓共同判断，并保存每类的多姿态样本特征。预测时取最接近的 3 个同类姿态计算距离，不再把侧躺、倾斜和正放样本压缩为单一类别质心。D435if 正式模型必须由真实 D435if 数据训练和验证，训练回放结果不等于泛化准确率。
...
输出的 `annotated-faces.png` 用不同颜色标出实际观测平面，`face-topology.json` 保存法向、邻接、二面角、三面汇聚数和证据质量。若一个应当清晰可见的平面被切成很多碎片，应先调整 D435if 距离、曝光、投射器和光照，再修改阈值。
...
.\.venv\Scripts\python.exe -m sorting_vision.cli `
  ...
  --frame-dir "data\rgbd-d435if\d435if-pilot-01\triangular_prism\<sample-id>" `
  --background-dir "data\rgbd-d435if\d435if-pilot-01\empty_tray\<sample-id>" `
  --rgbd-shape-model "models\stable\rgbd\geometry-rgbd-multipose-v4.npz" `
  --output-dir "output\rgbd-detect"
```

上例中的 v4 是原 D415 模型，只能用于迁移对照。完成 D435if 训练后，应把 `--rgbd-shape-model` 改为新的实验候选，并按批次与 v4 对照；通过推广门槛前不得驱动实际分拣。
...
## 6. 2026-09-05：D415 历史 ROI 优化版本

本节数据与结果来自旧 D415。旧环境复现组合为 `models/stable/rgbd/geometry-rgbd-multipose-v4.npz` 和 `config/d415-reviewed-20260905.yaml`。程序已增加空托盘参考约束：正常托盘平移继续跟随当前画面，检测区域异常扩大到参考面积1.5倍以上才限制到参考范围；深度仍用于分割、三维形状及吸附面检查。
...
融合模型使用 `rgbd_geometry_v4_fused_edges` 格式；旧模型格式兼容。该 D415 实验在403个可用单实例的嵌套留批评估中低于同样本旧特征基线，多物体组成回放也下降，因此候选未推广。D435if 迁移从 `config/d435if-initial.yaml` 开始，旧 v4 只作基线；不要因任何候选输出了类别而绕过 `DEPTH_INVALID`、`OCCLUDED`、`NO_GRASP_SURFACE` 或运动互锁。
