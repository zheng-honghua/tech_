...
> 当前目录已实现“俯视 RGB-D 主相机 + 侧置 UVC RGB 辅助相机”的软件同步、ChArUco 标定、三维包围体投影、侧视 ROI 分类、冲突拒识和安全降级。它仍处于实验阶段：没有比赛平台标定和独立留出数据，因此默认配置不启用双视角，当前稳定生产链路仍是单目 RGB-D。

# 智能分拣 RGB-D 视觉模块

面向智能分拣赛项的固定俯视 RGB-D 三维视觉系统。项目可完成托盘工作区定位、RGB-D 实例分割、颜色与立体形状识别、可见面拓扑分析、吸盘抓取位姿估计、目标选择以及 JSON/TCP 安全输出。

当前硬件为 Intel RealSense D435if，使用通用 RealSense SDK 适配器；普通 USB/UVC 摄像头可作为侧视形状证据。侧相机不生成物体数量、三维位置、抓取位姿或安全状态。没有有效主相机深度时，系统始终输出不可执行状态。原来的单目二维 `VisionPipeline` 仅用于兼容和显示，新项目的执行链路应使用 `VisionPipeline3D`。
...
| 类型 | 当前文件 | 用途与限制 |
| --- | --- | --- |
| D435if 初始配置 | [`config/d435if-initial.yaml`](config/d435if-initial.yaml) | 当前已验证取流入口；1920×1080 彩色、848×480 深度、30 FPS。识别与抓取阈值仍待 D435if 实拍复审。 |
...
- [双视角采集、标定与验收](docs/guides/双视角采集标定与验收.md)：侧机安装、成对采集、ChArUco 标定、联系表、融合诊断和推广门槛。
- [双视角形状与颜色数据采集完整教程](docs/guides/双视角形状与颜色数据采集完整教程.md)：三画面拍摄、形状/颜色独立真值、A/B/C实体、正式数据划分、多物块拍摄和人工审查。
- [三 AprilTag 双相机采集与标定程序](docs/guides/三AprilTag双相机采集与标定程序.md)：同步拍摄俯视 RGB/深度与侧视 RGB，并用两个对角固定标签和一个自由标签完成实时标定。
...
| 双相机成对采集与标定 | `dual-capture`、`dual-calibrate` | [双视角采集、标定与验收](docs/guides/双视角采集标定与验收.md) |
| 双视角联系表与推广判定 | `dual-review`、`dual-evaluate` | [双视角采集、标定与验收](docs/guides/双视角采集标定与验收.md) |
...
2. 相机、标定与配置：[`camera.py`](src/sorting_vision/camera.py)、[`rgbd.py`](src/sorting_vision/rgbd.py)、[`dual_view.py`](src/sorting_vision/dual_view.py)、[`calibration.py`](src/sorting_vision/calibration.py)、[`config.py`](src/sorting_vision/config.py)。
...
6. 数据、审查与测试：[`rgbd_dataset.py`](src/sorting_vision/rgbd_dataset.py)、[`rgbd_review.py`](src/sorting_vision/rgbd_review.py)、[`dual_validation.py`](src/sorting_vision/dual_validation.py)、[`tests/`](tests/)。
...
.\.venv\Scripts\python.exe -m sorting_vision.cli dual-calibrate `
  ...
  --platform-id temporary --tray-pose-index 19 `
  --output config\dual\temporary\calibration.json

.\.venv\Scripts\python.exe -m sorting_vision.cli camera-live `
  --source dual --background-dir data\empty-tray `
  ...
  --side-shape-model models\side-geometry.npz --platform-id temporary
```

`diagnostics.dual_view.fusion_state` 会给出 `FUSED`、`TOP_ONLY`、`SIDE_MISSING`、`SIDE_OCCLUDED`、`UNSYNCED`、`CALIBRATION_INVALID` 或 `CONFLICT`。无论哪种状态，侧视只可改变形状字段，不能绕过 `DEPTH_INVALID`、`OCCLUDED` 或 `NO_GRASP_SURFACE`。
...
双视角 11 类注册表、成对/多物体采集、人工审查、KNN/RTrees、Top‑2 三种融合器、MobileNetV3、OpenVINO/TensorRT 和推广门槛的完整命令见[双视角几何识别与安全融合](docs/guides/双视角几何识别与安全融合.md)。
